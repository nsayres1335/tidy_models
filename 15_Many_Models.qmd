...
Can use a racing approach. Using tune_race_anova() instead of tune_grid(). Only poorly performing candidates are dropped at each stage of the resampling.
...
# will take about an hour to run
# race_results <-
#    all_workflows %>%
#    workflow_map(
#       "tune_race_anova",
...
Can use all the same functions (e.g., *autoplot()*).

The racing approach estimated about 1,050 models, 8.33% of the full set (12,600 models) in full grid. As a result, racing is about 4.8 times faster.
...
The racing approach selected the same candidate parameters as the complete grid for only 42% of the models, but the performance metrics are very similar. The correlations of RMSE values was 0.968 and the rank corrlation was 0.951. This indicates that, within a model, there were multiple tuning parameter combinations that produce near idential results.

## 15.5 Finalizing A Model

To finalize: 1. pick a workflow, 2. extract model from set, 3. update params with best from tuning, 4. fit on training data, 5. evaluate on test data.
...
Plot shows good alignment between predicted and observed.