---
title: "Intro and Modeling Basics"
format: html
date: last-modified
execute:
    cache: true
...
Binding predictions to actual sale prices in the test set.
...
Calculating RMSE, R-squared, and MAE on the test set predictions.

```{r}
#| label: binary-classification-data

data(two_class_example)
tibble(two_class_example)
```

From modeldata package. Contains example pred from test data set with two classes.

```{r}
#| label: hard-class-binary-metrics

conf_mat(two_class_example, truth = truth, estimate = predicted)

# accuracy, matthew correlation coefficient, F1 metric
classification_metrics <- metric_set(accuracy, mcc, f_meas)
classification_metrics(two_class_example, truth = truth, estimate = predicted)
```

Confusion matrix and classification metrics for hard class predictions.

```{r}
#| label: prob-class-binary-metrics
...
ROC curve and AUC for probabilistic predictions.
...
Data with 3+ classes. These data are out-of-sample predictions associated w/ 10-fold CV
...
Examples of multiclass classification analogues of hard class metrics.

```{r}
#| label: multiclass-roc

roc_auc(hpc_cv, obs, VF, F, M, L)
roc_auc(hpc_cv, obs, VF, F, M, L, estimator = "macro_weighted")
```

Multiclass prob metrics.
...
Can use any of these metrics w/ dplyr groupings to summarize by resample or other grouping variables.


```{r}
#| label: autoplot-dplyr
...
dplyr syntax works for autoplot() as well.